...
public class QuarkPair : TypeQuark
{
    ...
    void Update()
    {
        ...
        Vector3 pos = transform.position - FindObjectOfType<Proton>().transform.position;

        GetComponent<Rigidbody>().AddForce(-pos.normalized * FindObjectOfType<AllQuarkScript>().forceSlider.value * 20000 / pos.magnitude * Time.deltaTime, ForceMode.Force);
