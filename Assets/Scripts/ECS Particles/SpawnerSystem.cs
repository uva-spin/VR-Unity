        ...
        Entities
            ...
            .ForEach((Entity entity, int entityInQueryIndex, in SpawnerComponent spawner, in LocalToWorld location) => {
                ...
                for (var i = 0; i < spawner.Count; i++) {
                    ...
                    commandBuffer.SetComponent(entityInQueryIndex, instance, new MovementComponent { RadiansPerSecond = math.radians(random.NextFloat(25.0F, 90.0F)) });
                    // commandBuffer.SetComponent(entityInQueryIndex, instance, new ParticleColor { Hue = 0f });
                }

                commandBuffer.DestroyEntity(entityInQueryIndex, entity); // delete the spawner entity because it has served its purpose
            }).ScheduleParallel();
